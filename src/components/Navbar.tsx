    ...
    return (
        <>
            <div className="p-px rounded-[45px] bg-gradient-to-r from-[#666666] via-[#000000] to-[#666666]">
                <div
                    className="flex max-md:justify-start justify-between items-center rounded-[45px] bg-gradient-to-r from-[#030202] via-[#464646] to-[#030202] px-[25px] py-4 text-white">
                    <div
                    ...
                    <div className="left flex items-center gap-2">
                        <img
                            src="https://grevrwdhbvzyevkdvoig.supabase.co/storage/v1/object/public/decorative//logo.webp" // adjust the path based on your setup (e.g., import if needed)
                            alt="Blackwood Logo"
                    ...
                    <div className="max-md:hidden">
                        {menuItems.map(item => (
                            <Link key={item.text} to={item.route} onClick={closeMenu}
                                  className="mx-[15px] text-white no-underline font-normal transition-colors duration-300 hover:text-[#ccc]">{item.text}</Link>
                        ))}
                    </div>
                ...
                >
                    {menuItems.map(item => (
                        <Link key={item.text} to={item.route} onClick={closeMenu}
                              className="text-white px-5 py-[15px] no-underline border-b border-[#333] hover:bg-[#222]">{item.text}</Link>
                    ))}
